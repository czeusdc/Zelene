...
async def verify_node(state: AgentState) -> dict:
    ...
    await sse_manager.broadcast(state["deployment_id"], "signal", {
        "type": "status",
        "title": f"I've verified {high_conf} of {len(signals)} signals with reasonable confidence.",
        "content": "A couple of signals conflict with historical market behavior. I've adjusted confidence accordingly.",
    })
